...
def classify_historical_scenarios(yield_df: pd.DataFrame, spreads: pd.DataFrame) -> pd.DataFrame:
    df = pd.DataFrame(index=yield_df.index)
    if "2Y" in yield_df.columns:
        df["d2y_20d"] = yield_df["2Y"].diff(20)
    if "10Y" in yield_df.columns:
        df["d10y_20d"] = yield_df["10Y"].diff(20)
    if "10Y-2Y" in spreads.columns:
        df["dslope_20d"] = spreads["10Y-2Y"].diff(20)

    if df.empty:
        return pd.DataFrame()

    def _label(row):
        d2 = row.get("d2y_20d", np.nan)
        d10 = row.get("d10y_20d", np.nan)
        ds = row.get("dslope_20d", np.nan)
        ...
        if d2 > 0 and d10 > 0:
            ...
            return "Bearish Selloff"

        if d2 < 0 and d10 > 0:
            return "Twist: Front Rally / Long Selloff"

        if d2 > 0 and d10 < 0:
            return "Twist: Front Selloff / Long Rally"

        return "Unclassified"

    df["Scenario"] = df.apply(_label, axis=1)
    df["2Y Direction"] = np.where(df.get("d2y_20d", 0) > 0, "Up", "Down")
    df["10Y Direction"] = np.where(df.get("d10y_20d", 0) > 0, "Up", "Down")
    df["Slope Direction"] = np.where(df.get("dslope_20d", 0) > 0, "Steepening", "Flattening")
    return df.dropna(how="all")

def summarize_historical_scenarios(scenario_df: pd.DataFrame, yield_df: pd.DataFrame) -> pd.DataFrame:
    if scenario_df is None or scenario_df.empty or "10Y" not in yield_df.columns:
        return pd.DataFrame()

    future_10y_change = yield_df["10Y"].shift(-20) - yield_df["10Y"]
    working = scenario_df.copy()
    working["Forward_10Y_20D_Change"] = future_10y_change.reindex(working.index)

    out = working.groupby("Scenario", dropna=True)["Forward_10Y_20D_Change"].agg(
        Count="count",
        Avg_Forward_10Y_20D_Change="mean",
        Median_Forward_10Y_20D_Change="median",
        Best_Forward_10Y_20D_Change="max",
        Worst_Forward_10Y_20D_Change="min",
    ).reset_index()

    if not out.empty:
        total = out["Count"].sum()
        out["Frequency_%"] = out["Count"] / total * 100
        out = out.sort_values(["Count", "Scenario"], ascending=[False, True])
    return out

def build_bull_bear_signal_table(scenario_df: pd.DataFrame) -> pd.DataFrame:
    if scenario_df is None or scenario_df.empty:
        return pd.DataFrame()

    mapping = {
        "Bull Steepener": "Bullish",
        "Bull Flattener": "Bullish",
        "Bullish Rally": "Bullish",
        "Bear Steepener": "Bearish",
        "Bear Flattener": "Bearish",
        "Bearish Selloff": "Bearish",
        "Twist: Front Rally / Long Selloff": "Mixed",
        "Twist: Front Selloff / Long Rally": "Mixed",
        "Unclassified": "Mixed",
    }

    temp = scenario_df.copy()
    temp["Signal_Bucket"] = temp["Scenario"].map(mapping).fillna("Mixed")
    return temp.groupby(["Signal_Bucket", "Scenario"]).size().reset_index(name="Observations").sort_values(
        ["Signal_Bucket", "Observations"], ascending=[True, False]
    )

def chart_historical_scenarios(summary_df: pd.DataFrame) -> Optional[go.Figure]:
    if summary_df is None or summary_df.empty:
        return None
    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=summary_df["Scenario"],
        y=summary_df["Frequency_%"],
        text=summary_df["Frequency_%"].round(1),
        textposition="outside",
        marker_color=COLORS["accent"]
    ))
    fig.update_xaxes(tickangle=-20)
    return create_chart_layout(fig, "Historical Scenario Frequency", "Frequency (%)", 540, "Historical Scenarios")

def chart_historical_outcomes(summary_df: pd.DataFrame) -> Optional[go.Figure]:
    if summary_df is None or summary_df.empty:
        return None
    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=summary_df["Scenario"],
        y=summary_df["Avg_Forward_10Y_20D_Change"],
        text=summary_df["Avg_Forward_10Y_20D_Change"].round(3),
        textposition="outside",
        marker_color=COLORS["accent3"]
    ))
    fig.add_hline(y=0, line_dash="dash", line_color=COLORS["muted"])
    fig.update_xaxes(tickangle=-20)
    return create_chart_layout(fig, "Forward 20-Day 10Y Outcome by Historical Scenario", "10Y Change", 540, "Historical Scenarios")
...
def chart_ohlc(ohlc_df: pd.DataFrame, ticker: str) -> Optional[go.Figure]:
    ...
    fig.update_layout(xaxis=dict(rangeslider=dict(visible=True), type="date"))
    return create_chart_layout(fig, f"OHLC | {ticker}", "Price", 700)
...
def chart_technical_panels(ohlc_df: pd.DataFrame, ticker: str) -> Optional[go.Figure]:
    ...
    fig = make_subplots(
        ...
        subplot_titles=(f"{ticker} Close, Trend and Volatility Bands", "RSI (14) Momentum", "MACD Trend Shift"),
    )
    ...
    return create_chart_layout(fig, f"Technical Analysis | {ticker}", height=980)
...
with top_tabs[3]:
        ...
        bull_bear_table = build_bull_bear_signal_table(scenario_df)

        st.markdown(
            '<div class="note-box"><b>Historical Scenario Analysis and Simulations</b><br><br>'
            'This section classifies real historical episodes into Bull Steepener, Bull Flattener, Bear Steepener, Bear Flattener, Bullish Rally, Bearish Selloff, and Twist regimes. '
            'It also measures how often each scenario occurred and what happened next to the 10Y yield over the following 20 trading days. '
            'This means the user now sees both scenario construction and scenario results rather than only a surface-level scenario sketch.'
            '</div>',
            unsafe_allow_html=True
        )

        fig_dir = chart_rate_dynamics(yield_df[selected_cols], spreads)
        if fig_dir:
            st.plotly_chart(fig_dir, use_container_width=True)
        else:
            st.info("Directional metrics could not be computed yet.")

        c1, c2 = st.columns([1, 1])
        with c1:
            fig_freq = chart_historical_scenarios(scenario_summary)
            if fig_freq:
                st.plotly_chart(fig_freq, use_container_width=True)
            else:
                st.info("Historical scenario frequency chart could not be built.")
        with c2:
            fig_outcomes = chart_historical_outcomes(scenario_summary)
            if fig_outcomes:
                st.plotly_chart(fig_outcomes, use_container_width=True)
            else:
                st.info("Historical outcome chart could not be built.")

        b1, b2 = st.columns([1, 1])
        with b1:
            st.subheader("Historical Scenario Summary")
            if scenario_summary is not None and not scenario_summary.empty:
                st.dataframe(scenario_summary.round(4), use_container_width=True, hide_index=True)
            else:
                st.warning("Historical scenario summary is empty.")
        with b2:
            st.subheader("Bullish / Bearish Signal Breakdown")
            if bull_bear_table is not None and not bull_bear_table.empty:
                st.dataframe(bull_bear_table, use_container_width=True, hide_index=True)
            else:
                st.warning("Bullish / bearish signal breakdown is empty.")

        st.markdown(
            '<div class="note-box"><b>Interpretation Guide</b><br><br>'
            '<b>Bullish scenarios</b> represent falling yields and supportive bond-price conditions. '
            '<b>Bearish scenarios</b> represent rising yields and adverse bond-price conditions. '
            '<b>Steepeners</b> and <b>flatteners</b> show whether the curve widened or compressed. '
            'The forward-outcome table helps the user evaluate whether each setup historically led to continuation, reversal, or weak follow-through in the 10Y yield.'
            '</div>',
            unsafe_allow_html=True
        )
...
with top_tabs[5]:
        ...
        with tech_tabs[0]:
            ...
            if ohlc_df is not None and not ohlc_df.empty:
                mc = st.columns(4)
                latest_close = float(ohlc_df["Close"].iloc[-1]) if "Close" in ohlc_df.columns else np.nan
                latest_rsi = float(ohlc_df["RSI"].iloc[-1]) if "RSI" in ohlc_df.columns else np.nan
                latest_macd = float(ohlc_df["MACD"].iloc[-1]) if "MACD" in ohlc_df.columns else np.nan
                latest_signal = float(ohlc_df["MACD_Signal"].iloc[-1]) if "MACD_Signal" in ohlc_df.columns else np.nan
                with mc[0]:
                    st.metric("Latest Close", f"{latest_close:.2f}" if np.isfinite(latest_close) else "N/A")
                with mc[1]:
                    st.metric("RSI (14)", f"{latest_rsi:.2f}" if np.isfinite(latest_rsi) else "N/A")
                with mc[2]:
                    st.metric("MACD", f"{latest_macd:.3f}" if np.isfinite(latest_macd) else "N/A")
                with mc[3]:
                    st.metric("MACD Signal", f"{latest_signal:.3f}" if np.isfinite(latest_signal) else "N/A")
        ...
        with tech_tabs[2]:
            st.markdown(
                ...
                The charts in this tab were enlarged to improve visibility and institutional readability.
                <b>OHLC and trend overlays</b> help the user see whether price is trending or losing strength,
                <b>RSI</b> highlights momentum exhaustion,
                <b>MACD</b> identifies trend-shift conditions,
                and <b>Bollinger Bands</b> show volatility envelopes.
                This tactical layer should be read together with the macro regime, the historical scenario engine, and curve-shape diagnostics.
                </div>
                """,
                unsafe_allow_html=True,
            )
