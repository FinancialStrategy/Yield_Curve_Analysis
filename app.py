...
'</div>'.format(
    ...
    'Inverted' if current_slope_val < 0 else 'Flattening' if current_slope_val < 50 else 'Normal',
    volatility_val
), 
unsafe_allow_html=True
)
...
'Impact on 10Y': ["{:+.1f} bps".format((s - yield_df['10Y'].iloc[-1]) * 100) for s in scenarios.values()]
})
